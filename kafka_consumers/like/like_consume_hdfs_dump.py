...
class HDFS_Consumer(BaseConsumer):
	...
	def flatten_msg(self, parsed_msg):
		msg = parsed_msg['data']['action'] + '\t' + \
				...
				"%.15f" % parsed_msg['data']['location']['latitude'] + '\t' +\
				"%.15f" % parsed_msg['data']['location']['longitude'] + '\t' +\
				...
				"%s" % parsed_msg['data']['timeviewed'] + '\n'
		return msg

	def handle_msg(self, parsed_msg):
		msg = self.flatten_msg(parsed_msg)
		self.ftmp.write(msg)
		print "Temp file size", self.ftmp.tell()
	...
	def flush_to_hdfs(self):
		...
		self.webhdfs.copyFromLocal(self.ftmp.name, "/user/photo_dump/like/%s.dat"%hdfs_name)
		self.ftmp.close()

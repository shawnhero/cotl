...
class GenerateUsers():
	...
	def write_es_geo(self, es_host='http://localhost:9200/', index_name="geos", doc_type='user_geos'):
		...
		try:
			es.delete_all(index_name, doc_type)
		except Exception as e:
			print "Error", e
		else:
		...
		documents = []
		for record in self.userGeos:
			doc = {'uid':record[0], 'locations':{'lat':record[1],'lon':record[2]}}
			documents.append(doc)
		es.bulk_index(index_name, doc_type, documents)
